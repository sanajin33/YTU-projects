...
int i,j;
    ...
    for (i = 0; i < row; i++) {
        ...
        if (matris[i] == NULL) {
            ...
            // Daha once ayrilmis bellegi serbest birak
            for (j = 0; j < i; j++) {
                free(matris[j]);
            }
            free(matris);
...
int elementHesapla(struct elementler elementler){
    ...
    //birbirlerini yok ediyorlar cikarta islemi sonucuna gore kalan element sayisi atanir
    ...
    // eger eksiP ve artiE den varsa kac tane element olustur= kucuk sayi kadar
...
int i;
    ...
    // sartlari kontrol ediyoruz , olmamsi gerek bir sey varsa direkt 0 dondurup cikar
...
int main() {
    ...
    int secim,secim2,secim3,kisi_sayisi,i,row,col,zorluk,kullaiciKonum , buul=0;
